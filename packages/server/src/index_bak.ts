...
import { getItemList, FetchType, fetchFnMap, getUserInfo, getItem } from 'hacker-news-fetch'
...
class HackerNewsServer {
  ...
  constructor() {
    this.server = new Server(this.description,
    {
      capabilities: {
        tools: {},
        resources: {
  ...
  setupTool() {
    this.server.setRequestHandler(ListToolsRequestSchema, async () => {
      return {
        tools: [
          ...
          {
            name: TOOL_NAME.HACKER_NEWS_ITEM,
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      if (request.params.name === TOOL_NAME.HACKER_NEWS_ITEM) {
